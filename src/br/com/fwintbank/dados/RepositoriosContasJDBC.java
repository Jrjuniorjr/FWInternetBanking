...
public class RepositoriosContasJDBC implements IRepContas {
    ...
    public void inserir(ContaAbstrata e) throws Exception {
        //String sql = "INSERT INTO TB_CONTA " + "(NUMERO, SALDO TB_CLIENTE_CPF,  TIPO)" + "VALUES(?, ?, ?, ?)";
    ...
    public void atualizar(ContaAbstrata e) throws Exception {
        //String sql = "UPDATE TB_CONTA SET NUMERO = ?, SALDO = ?, TB_CLIENTE_CPF, TIPO = ? WHERE NUMERO = ?";
    ...
    public ContaAbstrata procurar(String key) throws Exception {
        ...
        try {
            ...
            if (key != null) {
                ...
                if (rs.getInt("TIPO") == 2) {
                    conta = new ContaImposto(rs.getString("NUMERO"), rs.getDouble("SALDO"), null, ContasEnum.IMPOSTO);
                } else if (rs.getInt("TIPO") == 1) {
                    conta = new Conta(rs.getString("NUMERO"), rs.getDouble("SALDO"), null, ContasEnum.BONIFICADA);
                } else {

                    conta = new Conta(rs.getString("NUMERO"), rs.getDouble("SALDO"), null, ContasEnum.POUPANCA);
                }
